...
import { getError, updateErrorStatus, saveSolution, addToPendingSync, incrementUploadedSolutions, } from '../../storage/local-store.js';
...
export async function handleResolve(input) {
    ...
    if (input.upload) {
        if (isCloudEnabled()) {
            try {
                ...
                // 솔루션 업로드
                cloudSolutionId = await uploadSolution(cloudErrorId, input.resolution, input.resolutionCode);
