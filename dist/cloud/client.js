...
export function getSupabaseClient() {
    if (!isCloudEnabled()) {
        throw new McpError(ErrorCode.CloudUnavailable, 'Cloud is not configured. Set FIXHIVE_SUPABASE_URL and FIXHIVE_SUPABASE_KEY.');
    }
    if (supabaseInstance) {
 ...
 * 에러 검색 (텍스트 기반)
 */
export async function searchErrors(query, options) {
    const client = getSupabaseClient();
 ...
 */
export async function getErrorCandidates(options) {
    const client = getSupabaseClient();
    let queryBuilder = client
        .from('errors')
        .select('id, message');
 ...
 * 솔루션 업로드
 */
export async function uploadSolution(errorId, resolution, resolutionCode) {
    const client = getSupabaseClient();
    const config = loadConfig();
 ...
 * 에러에 대한 솔루션 가져오기
 */
export async function getSolutionsForError(errorId) {
    const client = getSupabaseClient();
    const { data, error } = await client
        .from('solutions')
        .select('*')
 ...
 * 솔루션 투표
 */
export async function voteSolution(knowledgeId, helpful) {
    const client = getSupabaseClient();
    const config = loadConfig();
    // 기존 투표 확인
 ...
 * 콘텐츠 신고
 */
export async function reportContent(knowledgeId, reason) {
    const client = getSupabaseClient();
    const config = loadConfig();
