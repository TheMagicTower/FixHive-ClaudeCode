...
function getServer() {
    if (!serverInstance) {
        throw new McpError(ErrorCode.InternalError, 'Server instance not initialized');
    }
 ...
 * LLM을 사용한 유사 에러 검색
 */
export async function findSimilarErrors(errorMessage, options) {
 ...
 * 폴백 텍스트 검색 (LLM 실패 시)
 */
async function fallbackTextSearch(errorMessage, candidates, limit) {
    // 간단한 키워드 매칭
 ...
 * LLM을 사용한 해결책 요약
 */
export async function summarizeSolutions(errorMessage, solutions) {
